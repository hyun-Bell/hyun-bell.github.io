...
export async function extractImageDimensionsFromUrl(imageUrl: string): Promise<ImageDimensions> {
  try {
    ...
    console.error(`Failed to extract dimensions from ${imageUrl}:`, error);
...
function extractJpegDimensions(jpegBytes: Uint8Array): ImageDimensions | null {
  ...
  while (currentOffset < jpegBytes.length - 8) {
    ...
    if (markerType !== undefined && START_OF_FRAME_MARKERS.includes(markerType)) {
...
function extractWebpDimensions(webpBytes: Uint8Array): ImageDimensions | null {
  ...
  const hasVp8Format =
    webpBytes[VP8_FORMAT_OFFSET] === 0x56 &&
  ...
  const imageWidth =
    ((webpBytes[DIMENSION_OFFSET]! | (webpBytes[DIMENSION_OFFSET + 1]! << 8)) & DIMENSION_MASK) + 1;
  const imageHeight =
    ((webpBytes[DIMENSION_OFFSET + 2]! | (webpBytes[DIMENSION_OFFSET + 3]! << 8)) &
      DIMENSION_MASK) +
    1;
