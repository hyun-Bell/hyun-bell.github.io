...
export function enhanceHtmlWithImageMetadata(
  ...
  let imageIndex = 0;
  const enhancedHtml = htmlContent.replace(/<img([^>]+)>/g, (imageTag, attributes) => {
...
function createEnhancedImageTag(attributes: string, imageData: ImageInfo): string {
  ...
  const blurAttribute = imageData.blurDataURL
    ? ` data-blur-placeholder="${imageData.blurDataURL}"`
    : '';
...
function applyFallbackImageDimensions(htmlContent: string): string {
  ...
  const FALLBACK_ASPECT_RATIO = calculateAspectRatio(
    FALLBACK_DIMENSIONS.width,
    FALLBACK_DIMENSIONS.height,
  );

  return htmlContent.replace(/<img([^>]+)>/g, (imageTag, attributes) =>
    ensureImageHasDimensions(imageTag, attributes, FALLBACK_DIMENSIONS, FALLBACK_ASPECT_RATIO),
  );
}
