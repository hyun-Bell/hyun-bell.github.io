...
function applyOptimalAspectRatio(imageElement: HTMLImageElement, imageIndex: number): void {
  ...
  if (import.meta.env.DEV) {
    console.log(`[Client] Applied aspect ratio ${calculatedAspectRatio} to image ${imageIndex + 1}`);
  }
}
...
function upgradeToNaturalAspectRatio(imageElement: HTMLImageElement, imageIndex: number): void {
  ...
  if (import.meta.env.DEV) {
    console.log(`[Client] Upgraded to natural aspect ratio ${naturalAspectRatio} for image ${imageIndex + 1}`);
  }
}

function removeBlurPlaceholder(imageElement: HTMLImageElement, blurPlaceholder: string | null): void {
  if (blurPlaceholder) {
